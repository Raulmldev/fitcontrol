...
class HealthWellnessAgent extends AIAgentBase {
  ...
  Future<AIAgentMessage> processQuery(UserQuery query, User user) async {
    ...
    if (queryLower.contains('presión') ||
        queryLower.contains('ritmo') ||
        queryLower.contains('vital')) {
      response = _generateVitalsResponse(query.query, user);
      messageType = MessageType.warning;
      suggestions = [
        'Ver mis parámetros vitales',
        'Registrar medición',
        'Tendencias de salud',
      ];
    } else if (queryLower.contains('peso') ||
        queryLower.contains('grasa') ||
        queryLower.contains('medidas')) {
      response = _generateBodyCompositionResponse(user);
      messageType = MessageType.insight;
    } else if (queryLower.contains('sueño') ||
        queryLower.contains('estrés') ||
        queryLower.contains('energía')) {
      response = _generateWellnessResponse(query.query);
      messageType = MessageType.recommendation;
    } else {
      response = '''
¡Hola! Soy el Dr. Antonio Vásquez, tu especialista en salud y bienestar.

Con 34 años de experiencia en medicina preventiva, estoy aquí para:

🏥 MONITOREAR tus parámetros vitales y corporales
⚠️ DETECTAR alertas tempranas de salud
📊 ANALIZAR tu composición corporal detallada
🧘 OPTIMIZAR tu bienestar holístico (sueño, estrés, energía)
🤝 COORDINAR con tu nutricionista y entrenador para salud integral
💊 GESTIONAR condiciones de salud específicas

¿Cómo te sientes hoy? ¿Qué aspecto de tu salud te gustaría revisar?
      ''';
      suggestions = [
        'Mis parámetros vitales',
...
He coordinado con tu entrenador para optimizar recuperación.
  ...
  String _generateVitalsResponse(String query, User user) {
    return '''
❤️ ANÁLISIS DE PARÁMETROS VITALES

Últimas mediciones registradas:

PRESIÓN ARTERIAL:
• Valor: Pendiente de registro
• Meta: < 120/80 mmHg
• Categoría: Por determinar

RITMO CARDÍACO:
• En reposo: Pendiente de registro
• Meta: 60-100 BPM
• Durante ejercicio: Depende de intensidad

SATURACIÓN DE OXÍGENO:
• Valor: Pendiente de registro
• Meta: > 95%
• Alerta: < 90% requiere atención médica

⚠️ RECUERDA: Registra tus signos vitales regularmente. Valores anormales requieren consulta médica.

He coordinado con tu entrenador para ajustar intensidades según tu FC máxima.
    ''';
  }

  String _generateBodyCompositionResponse(User user) {
    return '''
📏 ANÁLISIS DE COMPOSICIÓN CORPORAL
...
• Categoría IMC: ${user.bmiCategory}

INTERPRETACIÓN CLÍNICA:
${user.bmi < 18.5
        ? '''
⚠️ PESO BAJO
Riesgos: Deficiencias nutricionales, osteoporosis, sistema inmune debilitado.
Acción: Coordinado con nutricionista para aumento ponderal saludable.
'''
        : user.bmi < 25
        ? '''
✅ PESO SALUDABLE
Excelente rango para salud metabólica.
Enfoque: Mantener composición corporal óptima.
'''
        : user.bmi < 30
        ? '''
⚠️ SOBREPESO
Riesgo aumentado de diabetes tipo 2, hipertensión.
Acción: Plan integral con nutricionista y entrenador.
'''
        : '''
🔴 OBESIDAD
Riesgo significativo de enfermedades cardiovasculares.
Acción URGENTE: Consulta médica + plan multidisciplinario.
'''}

OBJETIVOS REALISTAS:
• Meta de peso: ${(user.bmi >= 25 ? user.weight * 0.92 : user.weight * 1.05).toStringAsFixed(1)} kg
• Timeline: 3-6 meses
• Pérdida/ganancia semanal: 0.5-1 kg
    ''';
  }

  String _generateWellnessResponse(String query) {
    return '''
🧘 ANÁLISIS DE BIENESTAR

SUEÑO 💤
Calidad del sueño es fundamental para:
...
• Pre-sueño: Sin pantallas 60 min antes
• Duración: 7-9 horas según edad

ESTRÉS 🧘‍♂️
Niveles de cortisol afectan:
• Recuperación
...
• Meditación mindfulness 10 min/día
• Yoga restaurativo

ENERGÍA ⚡
Patrones óptimos:
• Pico mañana 9-11am
• Estabilidad tarde 2-4pm
• Sin "crash" post-comida
    ''';
  }
