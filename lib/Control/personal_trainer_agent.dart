...
class PersonalTrainerAgent extends AIAgentBase {
  ...
  Future<AIAgentMessage> processQuery(UserQuery query, User user) async {
    ...
    if (queryLower.contains('ejercicio') ||
        queryLower.contains('entrenamiento') ||
        queryLower.contains('rutina')) {
      response = _generateWorkoutResponse(query.query, user);
      messageType = MessageType.recommendation;
      suggestions = [
        'Ver mi rutina de hoy',
        'Plan de entrenamiento semanal',
        'Ejercicios para ${user.fitnessGoal}',
      ];
    } else if (queryLower.contains('técnica') || queryLower.contains('forma')) {
      response = _generateTechniqueResponse(query.query);
      messageType = MessageType.text;
    } else if (queryLower.contains('progreso') ||
        queryLower.contains('evolución')) {
      response = _generateProgressResponse(user);
      messageType = MessageType.insight;
    } else {
      response = '''
¡Hola! Soy Carlos Rodríguez, tu entrenador personal en FitControl.

Con 35 años de experiencia entrenando atletas de todos los niveles, estoy aquí para:

🏋️ DISEÑAR tu plan de entrenamiento personalizado
📊 ANALIZAR tu progreso y adaptar la rutina
🎯 OPTIMIZAR tu técnica para máximos resultados
🤝 COORDINARME con tu nutricionista y experto en salud
🚑 PREVENIR lesiones y cuidar tu cuerpo
...
Ajustes calóricos según volumen de entrenamiento.
  ...
  String _generateWorkoutResponse(String query, User user) {
    return '''
🏋️ RUTINA PERSONALIZADA - ${user.fitnessGoal.toUpperCase()}

ENTRENAMIENTO DE HOY:
Basado en tu nivel ${user.activityLevel} y objetivo:
...
SERIE PRINCIPAL (${_getWorkoutDuration(user)} min):
${_getExercisesForGoal(user.fitnessGoal)}
...
💡 He coordinado con tu nutricionista: consume tu pre-entreno 90 min antes.
    ''';
  }

  String _generateTechniqueResponse(String query) {
    return '''
🎯 ANÁLISIS DE TÉCNICA

Para optimizar tu forma en cada ejercicio:

PRINCIPIOS FUNDAMENTALES:
• Control excéntrico (3-4 segundos)
• Rango completo de movimiento
• Tensión constante en el músculo objetivo
• Respiración coordinada

Recuerda: ¡La calidad supera a la cantidad! Mejor 8 repeticiones perfectas que 12 mediocres.

Puedo analizar videos de tu técnica si los subes a la app.
    ''';
  }

  String _generateProgressResponse(User user) {
    return '''
...
INDICADORES:
• Consistencia: Excelente (5/7 días)
• Intensidad promedio: Progresando
• Volumen semanal: Optimal para tu nivel
• Recuperación: Buena señal

RECOMENDACIONES:
1. Aumentar carga en press de banca 2.5kg
2. Mantener progresión actual en sentadilla
3. Incrementar cardio 5 min la próxima semana

¡Sigue así! Estás en el camino correcto para ${user.fitnessGoal}.
    ''';
  }

  String _getWorkoutDuration(User user) {
  ...
  String _getExercisesForGoal(String goal) {
    if (goal.toLowerCase().contains('fuerza')) {
      return '''• Sentadilla 4x5 (85% 1RM)
• Press banca 4x5 (85% 1RM)
• Peso muerto 3x5 (87% 1RM)
• Dominadas 4x6-8
• Press militar 3x6''';
    }

    if (goal.toLowerCase().contains('volumen') ||
        goal.toLowerCase().contains('masa')) {
      return '''• Press inclinado 4x8-12
...
• Extensiones de tríceps 3x12-15''';
    ...
    if (goal.toLowerCase().contains('pérdida') ||
        goal.toLowerCase().contains('definición')) {
      return '''• Circuito HIIT (30s trabajo/15s descanso)
• Burpees 3x15
• Mountain climbers 3x30s
• Kettlebell swings 3x20
• Sprint en el lugar 4x20s''';
    }

    return '''• Sentadilla corporal 3x15
• Flexiones 3x máximo
• Plancha 3x45s
